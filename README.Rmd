...
- We will first create a test dataset. SBF package has the function 
  ...
  number of different rows. All the matrices will have the same number of
  columns.
```{r}
set.seed(1231)
...
`optimizeFactorization` function. Depending upon the mymat and initial values,
...
if ((round(myopt$error,2) == round(myopt_inv$error,2)) && (round(myopt$error,2) == round(myopt_cor$error,2))) {
  cat("After optimization, for all three A-SBF factorizations, the final error is the same (up to 2 decimals)")
