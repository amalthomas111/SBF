...
the final factorization error is stored in `asbf$error`.
The initial factorization error we started with is stored in `asbf$error_start`.
...
asbf$error
```
The error decreased by a factor of `r round(asbf$error_start/asbf$error,2)`.
...
When calling `SBF`, `approximate = FALSE` computes the SBF factorization,
