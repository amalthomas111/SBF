...
1. Initialize $\Delta_i^{k}$,$V^{k}$ from JSB factorization.
Compute $\epsilon^{k,k,k} = {\sum^{N}_{i=1}\|D_i - U_i^{k} \Delta_i^{k} {V^{k}}^{\!T}\|^{2}}_F$
2. Update $\mathbf{U_i^{k+1}}$:\
...
This shows that the iterative update procedure converges to give the
