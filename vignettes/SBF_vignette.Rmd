...
Now, let us create two new matrix list containing our mymat matrix list.
We will add a dataset with similar variance to the first list and
...
Let's compute SBF with the new datasets
...
Let's take the newly estimated values $U_i$, $\Delta_i$, and $V$ for the four
...
e4 / e3
```
With inverse variance weighting, the deviation is less.
...
decomposition of $M$, where $M=V \Theta V^T$.
Once the $V$ and $\Delta_i$ is learned using the SBF factorization,
we compute $U_i$ with orthonormal columns using the proposition I.
The estimated $V$ space captures inter-tissue gene expression correlation
relationship.
For gene-expression analysis, we do not update/change $V$ while optimizing the
factorization error as we may not be able to interpret the new $V$ space.
So in the `optimizeFactorization` function, we set `optimizeV = FALSE` while
reducing the factorization error.
...
Check estimated Ui's have orthonormal columns
