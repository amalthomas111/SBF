...
- Estimate $V$ using sum of $D_i^T D_i / k$

- Estimate $V$ using sum of $D_i^T D_i / k$ with inverse variance weighting
...
Here the A-SBF call is made without minimizing the factorization error (`minimizeError=FALSE`).
```{r}
...
- 'asbf$u_ortho` is the closest matrix to exact U in SBF with orthonormal columns


`asbf$u_ortho` has orthonormal columns, and `asbf$v` is orthogonal.
...
```
This is the same error (no A-SBF optimization) we showed previously in A-SBF
examples. Now let us check the final decomposition error after optimization.
...
We can also compute the same by calling the `optimizeFactorization`. For 
example,
...
cat("\nFor asbf inv, # iteration =", asbf_inv$error_pos, "final error =",
    asbf_inv$error)
cat("\nFor asbf cor, # iteration =", asbf_cor$error_pos, "final error =",
    asbf_cor$error)
```
