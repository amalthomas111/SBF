...
for (sp in names(avg_counts)) {
  if (length(osbf_noVupdate$delta[[sp]]) == 1) {
    d_inv_NoVupdate[[sp]] <- as.matrix(diag(as.matrix(1 / osbf_noVupdate$delta[[sp]])))
    } else {
      d_inv_NoVupdate[[sp]] <- as.matrix(diag(1 / osbf_noVupdate$delta[[sp]]))
    }
}
d_inv <- list()
for (sp in names(avg_counts)) {
  if (length(osbf$delta[[sp]]) == 1) {
    d_inv[[sp]] <- as.matrix(diag(as.matrix(1 / osbf$delta[[sp]])))
    } else {
      d_inv[[sp]] <- as.matrix(diag(1 / osbf$delta[[sp]]))
    }
}
```
...
colnames(df_proj_avg_noVupdate) <- paste0("Dim", seq_len(ncol(df_proj_avg_noVupdate)))
...
colnames(df_proj_noVupdate) <- paste0("Dim", seq_len(ncol(df_proj_noVupdate)))
...
colnames(df_proj_avg) <- paste0("Dim", seq_len(ncol(df_proj_avg)))
