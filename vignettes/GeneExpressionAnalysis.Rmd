...
normalizeTPM <- function(rawCounts, method, gene_length = NULL,
                              priorcount = 0.01, min = 1) {
...
for (sp in species) {
  # reading raw counts
  counts <- read.table(paste0(path, "counts/", sp,
  ...
  gene_length_path <- "~/Dropbox/0.Analysis/0.paper/ensembl94_annotation/"
  gene_length <- read.table(paste0(gene_length_path, sp, "_genelength.tsv"),
