...
Reduce the tolerance threshold (default value 1e-10) to further minimize
the factorization error. Lowering threshold value increases the computing time.

The final factorization error and number of updates taken:
```{r}
cat("\n", sprintf("%-27s:","Final error [No V update]"), sprintf("%16.2f",
                                                        sbf_noVupdate$error))
cat("\n", sprintf("%-27s:","Final error [With V update]"), sprintf("%16.2f",sbf$error))
cat("\n", sprintf("%-27s:","# of update [No V update]"), sprintf("%16d",
                                                      sbf_noVupdate$error_pos))
cat("\n", sprintf("%-27s:","# of update [With V update]"), sprintf("%16d",sbf$error_pos))
```
...
Decomposition error vs. updates

```{r}
par(mfrow = c(1,3))
plot(x = 1:length(sbf$error_vec), y = sbf$error_vec,
...
Gene expression profiles cluster by tissue type indepedent of the species of
origin.
...
sel_colors <- brewer.pal(8,"Dark2")[1:length(unique(df_proj_noVupdate$tissue))]
i <- 1
j <- 2
...
The projected libraries in dimension 2 and 3.
...
ggplot(df_proj, aes(x = df_proj[, i],
  ...
  theme(legend.title = element_blank())
```
Similarly we could check for other dimensions of the common space.
We observe that both clustering and 2D projections plots in the common space
with optimized V is very similar to the one without V update.
So we will use the common space with optimized V for the future analysis.


# Expression specificity and eigengene loading
...
From 2D plot, we find that along dimension 2, testis profiles lie in the negative
axis and separate from the rest of the tissues. We will plot the relationship
between $U_i$ loadings in dimension 2 and testis expression.
...
Dimension 2 $U_i$ loading vs $\tau$ for humans
...
Dimension 2 $U_i$ loading vs $\tau$ for pig
...
Data link: https://figshare.com/s/d96c586d5e53199d5370
...
the background. You can files required for the GO analysis at this
link: 10.6084/m9.figshare.19633131
...
The testis-specific genes has negative loadings.
...
Barplot with enrichment and p-value
...
go_plot <- ggplot(go_out, aes(x = term, y = ratio, fill = padj)) + geom_col() +
  ...
  ylab(paste0("Ratio of genes in GO category (", species_scientific(species),")")) +
